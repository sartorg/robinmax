...
def robinmax_bac(graph, num_seeds, max_cover_size, thresh_budget=0,
                 max_thresh_dev=0.0, weight_budget=0.0, max_weight_dev=0.0,
                 max_time=3600, epsilon=1e-6,
                 debugging=False, disable_cuts=False, lp=False, covers=[],
                 thresholds=[], save_pruned=False, run_as_heuristic=False,
                 num_nodes=None, points_to_exclude=None, out_f=sys.__stdout__):
    ...
    run_as_heuristic: bool
        Put emphasis on heuristics.

    num_nodes: int or None
        Maximum number of nodes, or None if no limit.

    points_to_exclude: list() or None
        List of incidence vectors of seeds to be excluded, or None.
    ...
    if (num_nodes is not None):
        prob.parameters.mip.limits.nodes.set(num_nodes)
    ...
    if lp:
        ...
        prob.variables.add(
            ...
            names=['y_' + str(i) for i in range(graph.num_nodes)])
    if ((thresh_budget > 0 and max_thresh_dev > 0) or 
        (weight_budget > 0  and max_weight_dev > 0)):
        # We are solving the robust version of the problem
        pi_start = graph.num_nodes
        prob.variables.add(
        ...
        prob.variables.add(
            ...
            names=['mu_' + str(i) for i in range(graph.num_nodes)])
        z_start = mu_start + graph.num_nodes
        prob.variables.add(lb=[0], ub = [graph.num_nodes],
                           types=[prob.variables.type.continuous], names=['z'])
    ...
    # Exclusion of undesired incidence vectors of seed nodes
    if (points_to_exclude is not None):
        prob.linear_constraints.add(
            lin_expr=[cplex.SparsePair(
                ind=[i for i in range(graph.num_nodes)],
                val=[-1 if round(points_to_exclude[j][i]) == 1 else 1
                     for i in range(graph.num_nodes)])
                      for j in range(len(points_to_exclude))],
            senses=['G' for j in range(len(points_to_exclude))],
            rhs=[1 - sum(points_to_exclude[j][:graph.num_nodes])
                 for j in range(len(points_to_exclude))],
            names=['no_good_' + str(j) for j in range(len(points_to_exclude))])
    ...
    if (is_robust or run_as_heuristic):       
        lazy_constr = cb.LazyConstraint(
            ...
            max_weight_dev, lazy_constraints, lazy_info, pruned_info,
            epsilon, run_as_heuristic, debugging, out_f)
    ...
    try:
        ...
        lb_prob.variables.set_lower_bounds(
            [(i, prob.solution.get_values(i)) for i in range(graph.num_nodes)])
        # Solve lower bounding problem
        lb_prob.set_results_stream(None)
        lb_prob.set_log_stream(None)
        lp_prob.parameters.threads.set(1)
        lb_prob.solve()
        ...
        best_objective = prob.solution.get_objective_value()
        ...
        if (debugging or not run_as_heuristic):
            ...
            values = prob.solution.get_values(y_names)
    ...
    if debugging:
        # Save the problem, but first add all the generated lazy constraints
        for i, constraint in enumerate(lazy_constraints):
            prob.linear_constraints.add(
                lin_expr=constraint,
                senses=['G'], rhs=[0],
                names=['lazy_constraint_' + str(i)])
        prob.write('last_problem.lp')

    prob.end()

    return lazy_info, pruned_info, results
# -- end function


def robinmax_bac_restart(graph, num_seeds, max_cover_size,
